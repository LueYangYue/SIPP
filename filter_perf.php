...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_SESSION['id']) && isset($_GET['year'])) {
  ...
  if ($_SESSION['stud_year'] == '0') {
    try {
    $sql1 = "SELECT r.kod AS rk, r.pelajar, r.kursus, r.mata, r.status FROM prestasi AS r JOIN kursus AS k ON r.kursus = k.kod ";
    $sql2 = "WHERE NOT r.kursus = 'PNG' AND k.pensyarah = :id AND sesi = :acad_session ORDER BY r.pelajar ASC";
  ...
  try {
  $sql1 = "SELECT p.id AS pi, p.nama AS pn, l.status AS ls, k.kod AS kk, k.nama AS kn, r.kod AS rk, r.mata AS rm, r.status AS rs ";
